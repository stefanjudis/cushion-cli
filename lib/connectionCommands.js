...
connectionCommands._deleteAdmin = function(args, cli) {
  ...
  // @todo make this with a switch statement - looks better
  if (args.length === 0) {
    prompt('What is the name of the new admin? -> ', function(val) {
      name = val;
    ...
    console.log(
      'Use the \'deleteAdmin\' command without or with one argument (user)'
    );
...
connectionCommands._config = function(args, cli) {
  ...
  switch (argsLength) {
    ...
    default:
      console.log(
       'Please, use the \'config\' command 0, 1, 2 or 3 arguments.'
      );
