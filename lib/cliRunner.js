...
cli.handleConnectionCommands = function() {
  prompt(clc.blue('Connection -> '), function(val) {
    if (connectionCallbacks[val] !== undefined) {

      cli.runner = cli.handleConnectionCommands;
      cli.cushion[val](connectionCallbacks[val]);
...
cli.handleDatabaseCommands = function() {
  prompt(clc.blue('Database -> '), function(val) {
...
cli.initPrompt = function() {
  prompt('Where is the couchdb host? (127.0.0.1) -> ', function(val) {
    host = val || '127.0.0.1';

    prompt('What is the couchdb port? (5984) -> ', function(val) {
      port = val || '5984';

      cli.cushion = new cushion.Connection(host, port);
      cli.cushion.version(function (error, version) {
        if (error) {
          console.log(clc.red('Error appeared could not connect to couchdb'));
          cli.initPrompt();
        } else {
          cli.handleConnectionCommands();
        }
