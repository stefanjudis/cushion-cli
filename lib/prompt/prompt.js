...
cPrompt._promptGetUserPassword = function() {
  ...
  cPrompt.rl.question('What is the passwort for the user? ', function(password) {
 ...
 * with wich name. Callback will be called afterwards
 *
 * @param  {function} callback callback
 */
cPrompt._setupSaveConnection = function(callback) {
  cPrompt.rl.question('Do you want to save this connection for later? (yes|no) ', function(save) {
    if (save.match(/^[yY]*?(es)?$/)) {
      cPrompt.rl.question('What should be the name of new saved conntection? ', function(name) {
        if (name.length) {
