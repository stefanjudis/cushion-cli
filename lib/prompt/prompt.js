...
cPrompt._setCompleter = function(cli) {
  var completer = function(line, callback) {
        ...
        if (completion.hasOwnProperty(firstLevel)) {
          completion[firstLevel](linePartial, callback);
...
cPrompt._setupSavedConnection = function() {
  cPrompt.rl.question(
    ...
    'or choose one connection by entering depending number: ',
    function(number) {
      var connections = cPrompt.cli.connections,
          inputNumber = parseInt(number, 10) - 1,
          inputConnectionName = Object.keys(connections)[inputNumber],
      ...
      if (isNaN(inputNumber)) {
        if (number.length === 0) {
          ...
          console.log(clc.red(
            '\nInput was not a number. Let\'s start default setup.\n'
          ));
        }
        ...
        if (inputConnection) {
          cPrompt.cli.createConnection(
            ...
            inputConnection.host,
            inputConnection.port
          );
        } else {
          console.log(clc.red('\nUnfortunately your input doesn\'t match a connection\n'));
          cPrompt._showConnections(cPrompt._setupSavedConnection);
...
cPrompt._showConnections = function(callback) {
  var i = 1,
      connectionNames = Object.keys(cPrompt.cli.connections);
  ...
  if (connectionNames.length > 0) {
    ...
    console.log(clc.yellow('Unfortunately it\'s empty...'));
  }
