...
cPrompt.setup = function(cli) {
  ...
  cPrompt._showWelcomeScreen(cPrompt._promptCheckForUpdates);
};
  ...
  if (check.match(/^[yY]*?(es)?$/)) {
    npm.load(
      ...
      function(error, npm) {
        ...
        npm.search('cushion-cli', function(err, result) {
          console.log = log;
          if (err) {
            ...
            console.log(
              '\nThe most actual version is ' +
                clc.yellow(result['cushion-cli'].version) + '.\n'
            );

            cFile.readPackageJSON(function(error, data) {
              var npmConfig;
              ...
              if (error) {
                ...
                console.log(
                  'Your version is ' + clc.yellow(npmConfig.version) + '.'
                );
 ...
 * @private
 */
cPrompt._showWelcomeScreen = function(callback) {
