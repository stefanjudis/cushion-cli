...
helper._getAllCommands = function(object) {
  var commands = Object.keys(object).filter(
        function(command) {
          return (command[0] === '_' && command !== '_simpleCommand');
        }
      ),
      genCommands = Object.keys(generalCommands).filter(
        function(command) {
          return (command[0] === '_');
        }
      ),
  ...
  // concatenating level specific and general commands
  // after that remove underscore for displaying it to user
  allCommands = commands.concat(genCommands).map(function(command) {
