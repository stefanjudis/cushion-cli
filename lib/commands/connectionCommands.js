...
connectionCommands._database = function(input, cli) {
  ...
  if (name) {
    ...
    console.log(clc.red('\nYou must define a name for particular database.\n'));
  }
...
connectionCommands._deleteConnection = function(input, cli) {
  var index,
      connectionName = input[1],
      connectionIndex;

  if (input.length > 1) {
    // input by name
    if ( isNaN(+connectionName)) {
...
connectionCommands._saveConnection = function(input, cli) {
  ...
  if (argsLength > 0) {
    ...
    console.log('Please ' + clc.blue('ENTER') + ' name of connection as second parameter.');
...
// TODO show connections shoul maybe go in HERE
