...
connectionCommands._deleteConnection = function(input, cli) {
  ...
  if (input.length > 1) {
    ...
    if (+connectionName !== +connectionName) {
      this.__deleteConnectionWithName(connectionName, cli);
    } else {
      this.__deleteConnectionWithIndex(connectionName, cli);
    }
  } else {
...
connectionCommands.__deleteConnectionWithName = function(connectionName, cli) {
  ...
  this.__deleteConnectionWrite(connectionName, cli);
};


/**
 * Delete connection when input was typeof string
 * but parsing to number was possible
 *
 * @param  {String} connectionName connectionName
 * @param  {Object} cli            general cli object
 *
 * @private
 */
connectionCommands.__deleteConnectionWithIndex = function(connectionName, cli) {
  var connectionIndex = +connectionName - 1;
